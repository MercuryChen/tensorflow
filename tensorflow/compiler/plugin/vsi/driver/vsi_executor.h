...
#include "thrift/boss_data_type.h"
#include "thrift/boss_socket.h"
...
class VsiExecutor : public se::internal::StreamExecutorInterface {
 public:
  explicit VsiExecutor(std::shared_ptr<tim::vx::Context> vsiCtx,
                       const int device_ordinal, se::PluginConfig pluginConfig);
  ~VsiExecutor();
  ...
  //     // auto it = std::find_if(kVsiTensorContainer.begin(),
  //     kVsiTensorContainer.end(),
  ...
  se::internal::StreamExecutorInterface* GetUnderlyingExecutor() override {
    return this;
  }
 ...
 public:
  ...
  std::shared_ptr<TMultiplexedProtocol> multiplexed_protocol_;
  // create client
